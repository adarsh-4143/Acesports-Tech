    ...
    <>
      <article className="min-h-screen bg-white pt-32 pb-20">
        <div className="max-w-7xl mx-auto px-5">
          <div className="mb-8">
            <Link href="/blog" className="inline-flex items-center gap-2 text-[#39FF14] text-sm font-bold uppercase tracking-widest hover:text-slate-800 transition-colors bg-slate-50 px-5 py-2.5 rounded-full border border-slate-200 shadow-sm w-fit">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div className="lg:col-span-8">
              ...
              {/* 1. Photo */}
              <div className="rounded-3xl overflow-hidden shadow-lg border border-slate-100 mb-8">
              ...
              <div className="flex flex-wrap items-center gap-6 text-slate-600 text-sm mb-6 pb-6 border-b border-slate-100">
                <div className="flex items-center gap-3">
                  {blog.authorImg ? (
                    <img src={getAssetUrl(blog.authorImg) || ""} alt={author} className="w-12 h-12 rounded-full object-cover border-2 border-[#39FF14]" />
                  ) : (
                    <div className="w-12 h-12 rounded-full bg-slate-100 flex items-center justify-center border-2 border-[#39FF14]">
                      <User size={24} className="text-[#39FF14]" />
                    </div>
                  )}
              ...
              {/* 3. Heading */}
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-display font-extrabold text-slate-900 leading-tight mb-8">
                {blog.title}
              </h1>
              ...
              <div className="prose prose-lg prose-slate max-w-none">
                {blog.excerpt && (
                  <p 
                    className="text-xl md:text-2xl text-slate-800 font-bold leading-relaxed mb-8 pb-8 border-b border-slate-100"
              ...
              {blog.authorBio && (
                <div className="mt-12 pt-8 border-t border-slate-100 flex items-start gap-6">
                  <div className="w-20 h-20 shrink-0 rounded-full overflow-hidden border-2 border-[#39FF14] bg-slate-100">
                    {blog.authorImg ? (
                  ...
                  <div>
                    ...
                    {blog.authorRole && <p className="text-xs font-bold text-[#39FF14] uppercase tracking-widest mb-3">{blog.authorRole}</p>}
              ...
              <div className="mt-12 pt-8 border-t border-slate-100 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <button className="w-10 h-10 flex items-center justify-center rounded-full bg-slate-50 text-slate-600 hover:bg-[#39FF14] hover:text-[#060f1e] transition-colors border border-slate-200 shadow-sm hover:shadow-[#39FF14]/50">
                    <Share2 size={16} />
                  </button>
                </div>
              </div>
              ...
              <div className="sticky top-28">
                ...
                <div className="space-y-6">
                  {otherBlogs.slice(0, 4).map((related) => (
                    <Link key={related.id} href={`/blog/${related.slug}`} className="group flex gap-4 bg-slate-50 p-3 rounded-xl border border-slate-100 hover:border-[#39FF14]/50 transition-all hover:shadow-md">
                      ...
                      <div className="flex flex-col justify-center">
                        <p className="text-xs font-bold text-[#39FF14] uppercase tracking-wider mb-1 line-clamp-1">{related.created_by || "Admin"}</p>
          ...
          <div className="mt-24 pt-16 border-t border-slate-200">
            <div className="flex items-center justify-between mb-10">
              ...
              <Link href="/blog" className="text-xs font-bold text-[#39FF14] uppercase tracking-widest bg-slate-900 px-5 py-2.5 rounded hover:bg-slate-800 transition-colors">
                View All
              </Link>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {otherBlogs.slice(0, 3).reverse().map((more) => (
                <Link key={more.id} href={`/blog/${more.slug}`} className="group block bg-white rounded-2xl overflow-hidden border border-slate-200 hover:border-[#39FF14] transition-all hover:shadow-xl hover:shadow-[#39FF14]/10">
                  ...
                  <div className="p-6">
                    <p className="text-[10px] font-bold text-[#39FF14] uppercase tracking-widest mb-2">
